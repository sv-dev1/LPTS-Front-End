...
function DynamicTable(props) {
  ...
  const tdData = () => {
    return students.map(student => {
     ...
     //   debugger
        // console.log("student.allInProgress" , student.allInProgress)
        var found = -1;
        let lastInprogressTarget = -1;
        if(student.allInProgress){
          var found = student.allInProgress.findIndex(e => e == learnTarget)

        }
        // console.log("found.allInProgress1" , found)
        if (student.inProgressingTarget) {
          //      debugger
          let inProgressingTarget = student.inProgressingTarget.split(',')
          lastInprogressTarget = Math.max(...inProgressingTarget) ;
          if(!Number.isFinite(lastInprogressTarget)){
             lastInprogressTarget = -1; 
          }
          ...
          if (indexForinProgressing > -1) {
            return 'IP'
          }else if(lastInprogressTarget > 0 &&  learnTarget < lastInprogressTarget  || student.inProgressTarget == 0 && learnTarget <= student.completedTarget ){
            return 'P'
          } else{
            return "";
          }
        } else if (student.inProgressTarget === learnTarget || found > -1) {
          return 'IP'
        } else if (learnTarget < parseInt(student.inProgressTarget) || learnTarget <= parseInt(student.completedTarget)) {
          return 'P'
        } else {
          return ''
        }
                ...
                return (
                  <>
                    <td className="unselectable" style={{textAlign: 'left'}}>
                      <Link
                        className="link-text diable-link"
                        style={{
                          backgroundColor:
                            myData(
