...
const SplitLearningTarget = (props) => {
  const { Option } = Select;
  const initialValues = {
  ...
  useEffect(() => {
    ...
    inputs.subjectId &&
      fetch(`${baseUrl}/Phases/GetAll?subjectId=${parseInt(inputs.subjectId)}&schoolId=${parseInt(schoolID)}` , {headers})
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    ...
    inputs.subjectId &&
      fetch(`${baseUrl}/Categories/GetAll?subjectId=${parseInt(inputs.subjectId)}&phaseId=${parseInt(inputs.phaseId)}&schoolId=${parseInt(schoolID)}` ,{headers})
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    ...
    >
      ...
      <Form.Item
        ...
        name="subjectId"
      >
        <Select disabled={true} defaultValue={inputs.subjectName} onChange={(value) => setInputs({ ...inputs, subjectId: value })} name='subjectId' style={{ width: 120 }} >
          {
      ...
      <Form.Item
        ...
        name="phaseId"
      >
        <Select disabled={true} defaultValue={inputs.phaseName} onChange={(value) => setInputs({ ...inputs, phaseId: value })} name='phaseId' style={{ width: 120 }} >
          {
      ...
      <Form.Item
        ...
        name="categoryId"
      >
        <Select disabled={true} defaultValue={inputs.categoryName} onChange={(value) => setInputs({ ...inputs, categoryId: value })} name='categoryId' style={{ width: 120 }} >
          {
...
export default Form.create()(SplitLearningTarget);